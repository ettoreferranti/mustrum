...
| E13-3 | MCP server adapter (`mustrum mcp`, stdio, `mustrum/mcp/server.py`): read-only tools `search_library`/`get_source`/`get_idea`/`list_citations` — raw structured reads only, deliberately not a grounded-QA tool (ADR-19, user-confirmed); zero core changes, no LLM call anywhere in this story | S | done |
