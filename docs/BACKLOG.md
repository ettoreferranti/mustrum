...
| E13-4 | MCP resource exposure: every source/idea listed as an individually-readable `mustrum://sources\|ideas/{id}` resource (`mustrum/mcp/server.py`), not only reachable via `get_source`/`get_idea` tool calls — ids are a startup snapshot, content still read live (ADR-20) | C | done |
