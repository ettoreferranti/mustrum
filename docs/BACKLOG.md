...
| E11-2 | GUI: tags editing (add/remove on sources and ideas), contact links (link a contact to a source/idea with a reason, `mustrum contact link` counterpart), audit upload (`POST /api/audit`, `mustrum audit` counterpart) ([PR #16](https://github.com/ettoreferranti/mustrum/pull/16)) | C | done |
...
| E11-8 | Pre-release security hardening (ADR-25): escape library data before `innerHTML` in the generated graph page (closes a stored-XSS-into-the-unauthenticated-local-API hole; SPA already escaped); cross-origin-write guard middleware refusing browser writes from other sites to the loopback API; corrupt-PDF / non-UTF-8 / offline inputs fail cleanly (CLI message + exit 1, GUI 422/502) instead of a raw traceback or opaque 500 — from an architecture/security/quality review ahead of early-user trials ([PR #21](https://github.com/ettoreferranti/mustrum/pull/21)) | S | done |
