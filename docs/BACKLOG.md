...
| ID | Story | Prio | Status |
|---|---|---|---|
| E13-1 | Core query service (`core/services/query.py::QueryService`, scoped to sources): retrieval = FTS5 search (E8-1) ∪ embedding cosine-similarity, capped at top_k; answer synthesised across all candidates in one LLM call via a new multi-source grounded primitive (`services/grounded.py::run_grounded_multi`, ADR-17) — each claim's quote verified against the *specific* source it's attributed to, `found:bool` a trusted classifier whose prose is discarded when false; `core/verify.py` unchanged. Idea/contact grounding and any CLI/GUI surface deliberately out of scope (E13-2/E13-3) | M | done |
| E13-2 | `mustrum chat` — conversational grounded Q&A (CLI REPL + GUI Chat tab): multi-turn session state (in-memory per session), every turn re-grounds via E13-1 rather than trusting prior conversational turns, ungrounded answers rejected/flagged same as summarisation; needs an ADR on how prior turns are allowed to shape retrieval without letting the model answer from un-verified chat history | M | todo |
| E13-3 | MCP server adapter (`mustrum mcp` or `adapters/mcp.py`): read-only tools (`search_library`, `get_source`, `get_idea`, `list_citations`) over E13-1's query service — adapter-only, no core changes, same grounding guarantees as chat/CLI | S | todo |
| E13-4 | MCP resource exposure for direct source/idea reads as MCP resources (not just tool calls) — depends on E13-3 | C | todo |
